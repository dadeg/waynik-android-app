...
public class MixedPositionProvider extends PositionProvider implements LocationListener, GpsStatus.Listener {
    ...
    private Runnable updateHandlerTask;
    ...
    public void startUpdates() {
        ...
        if (emergency) {
            ...
            turnOnListener(period);
            return;
        }
        saveBatteryByTurningListenerOnAndOff();
    }

    /*
    * an attempt to save battery.
    * */
    private void saveBatteryByTurningListenerOnAndOff() {


        updateHandlerTask = new Runnable() {
            @Override
            public void run() {
                turnOnListener(0);
                updateHandler.postDelayed(updateHandlerTask, period);
            }
        };

        updateHandlerTask.run();
    }

    private void turnOnListener(long period) {
    ...
    public void stopUpdates() {
        ...
        locationManager.removeGpsStatusListener(this);
        stopBackupProvider();
        updateHandler.removeCallbacks(updateHandlerTask);
    }
