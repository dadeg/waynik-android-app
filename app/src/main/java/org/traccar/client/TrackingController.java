...
public class TrackingController implements PositionProvider.PositionListener, NetworkManager.NetworkHandler {
    ...
    public TrackingController(Context context) {
        ...
        apiToken = preferences.getString(MainActivity.KEY_TOKEN, null);
    ...
    private void read() {
        ...
        databaseHelper.selectPositionAsync(new DatabaseHelper.DatabaseHandler<Position>() {
            ...
            public void onComplete(boolean success, Position result) {
                if (success) {
                    if (result != null) {
                        if (result.getEmail().equals(preferences.getString(MainActivity.KEY_EMAIL, null))) {
                            send(result);
                        } else {
                            delete(result);
                        }
    ...
    private void send(final Position position) {
        log("send", position);
        lock();
        String request = ProtocolFormatter.formatRequest(email, apiToken, position);
